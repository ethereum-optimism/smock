...
subtask(TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOB_FOR_FILE, async(_ , __ , runSuper) => {
    const compilationJob = await runSuper()
    compilationJob.solidityConfig.settings.outputSelection['*']['*'].push('storageLayout')
    return compilationJob
})

subtask(TASK_COMPILE_SOLIDITY_GET_ARTIFACT_FROM_COMPILATION_OUTPUT, async( { contractOutput }: { contractOutput: any } , __ , runSuper) => {
